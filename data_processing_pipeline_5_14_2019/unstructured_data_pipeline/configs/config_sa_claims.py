...
config_sa_claims
~~~~~~~~~~~~~~~~
Configuration file for SA Claims.
"""
from enum import Enum
from configparser import ConfigParser

class SAClaimsConfigFile(Enum):
    """SA Claims Data Pipeline Configuration"""
    CONFIG_FILE = 'C:\\Users\\wmurphy\\PycharmProjects' \
                  '\\data_processing_pipeline_5_14_2019' \
                  '\\config_files\\config_sa.ini'


class SAClaimsConfiguration(object):
    # create a ConfigParser instance
    config = ConfigParser()
    config.read(SAClaimsConfigFile.CONFIG_FILE.value)
    ...
    def get_raw_data(self) -> str:
        raw_data = self.config.get(self.sections[1], 'SA_CLAIMS')
        return raw_data

    @property
    def get_pickle_path(self) -> str:
        pickle_path = self.config.get(self.sections[2], 'SA_CLAIMS_PICKLE')
        # return path to where bda pipeline pickle files are stored
        return pickle_path

    @property
    def get_mapping_path(self) -> str:
        mapping_path = self.config.get(self.sections[3], 'SA_CLAIMS_MAPPING_PATH')
        return mapping_path
    ...
    def get_write_paths(self) -> dict:
        eml = self.config.get(self.sections[5], 'SA_CLAIMS_EML_WRITE_PATH')
        rtf = self.config.get(self.sections[5], 'SA_CLAIMS_RTF_WRITE_PATH')
        doc = self.config.get(self.sections[5], 'SA_CLAIMS_DOC_WRITE_PATH')
        docx = self.config.get(self.sections[5], 'SA_CLAIMS_DOCX_WRITE_PATH')
        pdf = self.config.get(self.sections[5], 'SA_CLAIMS_PDF_WRITE_PATH')
        doc_to_csv = self.config.get(self.sections[5], 'SA_CLAIMS_DOC_TO_CSV')
        pdf_img = self.config.get(self.sections[5], 'SA_CLAIMS_PDF_IMG')
...
# create an instance of the sa claims configuration file
sa_config = SAClaimsConfiguration()
